...
pub fn estimate_vigenere_period(text: &str, max_period: usize) -> Vec<(usize, f32)> {
    ...
    for p in 1..=max_period.min(chars.len() / 2).max(1) {
        let mut sum_ioc = 0.0;
        for i in 0..p {
            // Correct column selection: skip i first, then take every p-th element
            let column: String = chars.iter().skip(i).step_by(p).collect();
